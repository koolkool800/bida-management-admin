...
const RevenueChartDetail = ({ items }: { items: ProductStatistic[] }) => {
  // ** Hook
  ...
  const transformData = (data: ProductStatistic[]) => {
    const hihi = [
      {
        date: '2024-05-23',
        total_price_sum: 23611,
        total_product_price_sum: 20000
      },
      {
        date: '2024-05-24',
        total_price_sum: 23611,
    ...
    const totalProductPriceSumItems = data?.map(item => item?.total_product_price_sum)
    const totalPriceSumItems = data?.map(item => item?.total_price_sum)

    return {
      dates,
      totalProductPriceSumItems,
      totalPriceSumItems
    }
  }
  const { dates, totalProductPriceSumItems, totalPriceSumItems } = transformData(items)
  ...
  const series = [
    {
      name: 'Tổng tiền đồ ăn',
      data: totalProductPriceSumItems
    },
    {
      name: 'Tổng tiền hóa đơn',
      data: totalPriceSumItems
    }
  ]
  return (
    <Card>
