...
import { RecentInvoice } from 'src/types/dashboard'
...
const RecentOrders = ({ items }: Props) => {
      ...
      <Box sx={{ width: '100%' }}>
        ...
        <CardContent sx={{ pb: theme => `${theme.spacing(5.5)} !important` }}>
          {items?.map((item, index: number) => {
            return (
              <Box key={index} sx={{ display: 'flex', alignItems: 'center', mb: index !== items.length - 1 ? 6 : 0 }}>
                <Box
                  sx={{
                    ml: 4,
                    width: '100%',
                ...
                >
                  <Box sx={{ marginRight: 2, display: 'flex', flexDirection: 'column' }}>
                    ...
                    <Typography variant='caption'>HD{item.id}</Typography>
                  </Box>
